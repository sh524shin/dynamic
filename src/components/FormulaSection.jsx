...
const FormulaCard = ({ title, formula, description, badge }) => (
  <motion.div 
    ...
    className="shield-card-container"
  >
    {/* Top Accent Bar */}
    ...
    <div className="shield-card-body">
      {/* Visual Ambient Glow */}
      <div className="absolute top-0 right-0 w-32 h-32 bg-primary/20 rounded-full blur-[80px] opacity-40 group-hover:opacity-70 transition-all pointer-events-none" />
      
      <div className="flex justify-between items-start relative z-10">
        <h3 className="text-2xl font-black text-white tracking-tight drop-shadow-lg">{title}</h3>
        {badge && (
      ...
      <div className="relative z-10 bg-slate-900/60 p-8 rounded-2xl border border-white/10 shadow-inner my-2 flex items-center justify-center min-h-[140px] group-hover:bg-slate-900/80 transition-colors">
        <div className="text-white scale-110 md:scale-125 origin-center text-center w-full drop-shadow-[0_0_8px_rgba(255,255,255,0.2)] font-math">
  ...
  return (
    <div className="space-y-20 pb-20">
      <div className="text-center mb-24">
        ...
        <p className="text-text-muted text-lg max-w-2xl mx-auto">고등학교 수학 I 및 미적분 과정의 삼각함수 핵심 공식을 정리했습니다.</p>
      </div>
      ...
      {/* Strategy Card */}
      <div className="glass p-10 border-l-8 border-l-primary bg-primary/5 mt-32 flex flex-col md:flex-row gap-6 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 text-sm text-text-muted leading-relaxed">
            <div>
              <p className="font-bold text-neon-cyan mb-2 border-b border-white/10 pb-1">수학 I (공통)</p>
              **동경**의 위치와 주기성을 이해하는 것이 핵심입니다. **각 변환** 시에는 n의 홀짝에 따른 함수 변화 법칙만 알면 모든 공식을 유도할 수 있습니다.
            </div>
            <div>
              <p className="font-bold text-neon-purple mb-2 border-b border-white/10 pb-1">미적분 (선택)</p>
              덧셈정리는 모든 미분/적분 공식의 시작입니다. 유도 과정을 파악하면 고난도 배각/반각 공식을 더 쉽게 기억할 수 있습니다.
            </div>
          </div>
        </div>
      </div>
